  ...
  cellHeight: 40,
	...
	handleEvent: function (e) {
		if (e.type == 'touchstart') {
			this.lockScreen(e);
			if (e.currentTarget.id == 'sw-cancel' || e.currentTarget.id == 'sw-done' || e.currentTarget.id == 'sw-buttonl' || e.currentTarget.id == 'sw-buttonr') {
				if(this.device=='a'){
					this.tapUp(e);
				}else{
					this.tapDown(e);
	...
	create: function () {
		...
		div.innerHTML = '<div id="sw-header"><div id="sw-cancel">Cancel</' + 'div><div id="sw-buttonl"></' + 'div><div id="sw-buttonr"></' + 'div><div id="sw-done">Done</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
	...
	tapUp: function (e) {
		...
		if (e.currentTarget.id == 'sw-cancel') {
			...
			if (e.currentTarget.id == 'sw-done') {
				...
				if (e.currentTarget.id == 'sw-buttonl') {
					this.buttonlAction();
				} else {
					this.buttonrAction();
				}
	...
	setDevice: function (c) {
		this.device = c;
	},

	setButtonTexts: function (c1,c2,c3,c4) {
		if(c1!=null){
			if(c1!=''){
				document.getElementById('sw-cancel').innerHTML=c1;
			}else{
		...
		if(c2!=null){
			if(c2!=''){
				document.getElementById('sw-done').innerHTML=c2;
			}else{
		...
		if(c3!=null){
			if(c3!=''){
				document.getElementById('sw-buttonl').innerHTML=c3;
			}else{
		...
		if(c4!=null){
			if(c4!=''){
				document.getElementById('sw-buttonr').innerHTML=c4;
			}else{
	...
	setButtonlAction: function (action) {
		this.buttonlAction = action;
	},
	
	setButtonrAction: function (action) {
		this.buttonrAction = action;
	},
